...
public class AIControl : MonoBehaviour
{
    ...
    public void DetectNewAttractor(Vector3 location)
    {
        if (Vector3.Distance(location, this.transform.position) < detectionRadius)
        {
            Vector3 attractDirection = (location - this.transform.position).normalized;
            Vector3 newGoal = this.transform.position + attractDirection * attractRadius;
